...
function calculate() {
  ...
  let x = Math.pow(1 + interest, payments); /* Math.pow() calcula potências*/

  let monthly =
    (principal * x * interest) /
    (x -
      1); /* Se o resultado é um número finito, a entrada do usuário estava correta e // temos resultados significativos para exibir*/
  if (isFinite(monthly)) {
...
/* Tenta restaurar os campos de entrada automaticamente quando o documento é carregado/ pela primeira vez.*/ window.onload =
  function () {
...
function getLenders(amount, apr, years, zipcode) {
  ...
  if (!ad) return; // Encerra se não há ponto de saída // Codifica a entrada do usuário como parâmetros de consulta em um URL
  let url =
...
function chart(principal, interest, monthly, payments) {
  ...
  // Para baixo, até o canto// inferior direito
  ...
  for (let p = 1; p <= payments; p++) {
    ...
    // Desenha a linha até esse ponto
  }
  g.lineWidth = 3;
